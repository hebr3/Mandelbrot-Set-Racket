...
(define t3 (time
            (for*/list : (Listof (List Integer Integer Integer))
              ([x (range WIDTH)] [y (range WIDTH)])
              (list x y (mandelbrot (fl- (fl* SCALE (->fl x)) x0)
                                    (fl- y0 (fl* SCALE (->fl y))))))))
(time
 (for ([pt t3])
   (send dc set-pixel (first pt) (second pt)
         (rgb (third pt)))))
...
(define depth-list
  (map (λ ([x : (Listof Integer)]) (third x)) t3))
